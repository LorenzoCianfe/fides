...
| 0031 | Audit tail anchoring by signed, published high-water marks (refines 0001, 0024, 0028) | Accepted |
...
| 0.15.0 | 2026-08-07 | Phase 1 remediation, Slice 11: audit tail anchoring (ADR-0031). Closes the deferral ADR-0024 recorded against itself. The hash chain proves that what it *contains* was not altered but says nothing about what it no longer contains, so deleting the newest records left a shorter chain that verified perfectly — the one tamper an audit trail exists to catch. The head's `(seq, hash)` is now signed on an interval and **published to the process log**, with a copy in a new `audit_anchors` table (migration `0013`). The published line is the control and the table is convenience: whoever can truncate the trail can delete the rows in the same transaction, while a shipped log line cannot be unpublished. Signing is Ed25519 through a new KMS-shaped `SigningPort`, not an HMAC, because anyone able to verify an HMAC can forge one — an anchor must be checkable by someone who has stopped trusting this system; the verification key is public and logged at startup. `GET /v1/admin/audit/verify` now reports chain and tail separately with `ok` as their conjunction, `unanchored` is an explicit non-failure state, and `POST /v1/admin/audit/verify-anchor` verifies against an anchor held from a log archive. New settings `AUDIT_ANCHOR_KEYS` (required, no default) and `AUDIT_ANCHOR_INTERVAL_MS`. 341 tests. |
