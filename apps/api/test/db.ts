...
export async function resetDb(db: TestDatabase): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE transaction_history, postings, journal_entries, balances, ledger_accounts, kyc_applications, sessions, credentials, devices, webauthn_challenges, enrolment_tokens, email_verifications, users, idempotency_keys, outbox RESTART IDENTITY CASCADE`,
  );
}
