...
- **The CSRF cookie is `Path=/`, unlike the two credentials.** It has to be: `document.cookie` exposes only cookies whose path is a prefix of the *document's* path, and the client that must echo it is served from `/`, never from `/v1`. Scoped to the API's path it is invisible to the only code that needs it, and every state-changing request fails — which is what happened until the end-to-end suite drove a real browser through the flow. Widening it costs nothing: the token carries no authority on its own, and presenting it without the httpOnly access cookie proves nothing. The credentials keep their tight scoping, which is where the scoping is worth having.
...
Implemented (Phase 1 remediation, Slice 10, ADR-0029): **per-account lockout, and a trail for denied attempts.**

- **Lockout counts both factors against one counter** (default five attempts, fifteen-minute window, both configurable). A wrong password and a wrong TOTP code advance the same total, because an attacker past the password step is further along, not safer — giving the second factor its own budget would invert the intended difficulty. Where ADR-0025 relied on an in-memory, per-IP throttle that resets on restart and distributes across addresses, guessing against a known operator address is now stopped rather than slowed.
- **The counter and the denial record are written in a transaction of their own**, immediately after the failure. This is the documented exception to ADR-0024's rule that every audit record commits inside its action's transaction, and it is an exception by necessity: a denial has no committed action to be atomic with, and `verifyMfa` deliberately rolls its transaction back so a mistyped code does not also spend the login challenge. Anything written inside it would be discarded — which is exactly why failed second-factor attempts previously went uncounted.
- **Only a rejected code counts.** A stale or already-consumed challenge is not a guess at the second factor, and counting it would let anyone holding a spent token lock an operator out on demand.
- **The lock clears only when both factors have succeeded**, in the same statement that records the sign-in. Clearing it at the password step would let an attacker who already holds the password reset the counter at will and grind the code indefinitely.
- **A locked account fails with the same generic `Invalid credentials`** as every other rejection, so the response neither confirms the address nor signals that guessing is having an effect.
- **Denials are recorded only for a known admin.** An unknown address has no admin to reference and is itself personal data, which an un-erasable trail must not hold (ADR-0024). A spray across unknown addresses therefore leaves no audit trail and remains the throttle's problem — a real limitation, accepted rather than solved by writing PII into an immutable log.
- Recording is best-effort and never converts a rejected sign-in into a server error, which would itself signal that the attempt was interesting.

Implemented (Phase 1 remediation, Slice 10 Wave B, ADR-0030): **credential rotation and recovery.** Before this, a lost authenticator or a suspected password compromise had no remedy short of a direct `UPDATE` on the `admins` table — the unaudited, unilateral intervention the back office exists to make unnecessary.

- **Self-service password rotation** (`POST /v1/admin/me/password`) re-proves **both** factors: the current password, and a fresh TOTP code. The session alone must never suffice, or a stolen session becomes permanent account takeover; the password alone must not either, since it is the credential being replaced. The route sits behind authentication and **no permission** — rotating your own credential is not a capability the matrix may withhold — and is throttled like the login routes, because it verifies a credential and is therefore a guessing surface.
- **The code advances the same replay guard sign-in advances**, so a code spent rotating cannot afterwards be spent signing in. The visible cost is that a code just used to sign in is rejected and the next time step must be awaited; two independent guards, each blind to what the other spent, would be strictly weaker.
- **A successful rotation revokes every other session the operator holds**, sparing the calling one. A password change that leaves the attacker's session alive is bookkeeping, not containment.
- **Failures count against the ADR-0029 lockout** and are audited as denials carrying `operation: password_change`, so a rotation failure is distinguishable from a failed sign-in. Success clears the counter — the second and only other place both factors are satisfied.
- **Second-factor reset is behind four-eyes** (`admin_totp_reset`, the second registered `pending_admin_actions` type). A reset is a second-factor bypass by definition, since whoever enrols next holds the account, so it may never be unilateral. The maker half (`admin_totp_reset.request`) is held by the compliance officer alone — deliberately narrower than the funding maker half, because funding credits a customer within a cap while a reset hands over a back-office identity — and the checker half by the super-admin alone, which is again denied the maker half. Both are in `SEGREGATED_PERMISSION_PAIRS`, so the matrix-wide invariant test covers the new pair automatically.
- **Approval clears the secret, the enrolment marker, the replay guard, and the lockout, and revokes all of the target's sessions**, inside the transaction that decides the request. Each element earns its place: enrolment because re-enrolment refuses an enrolled admin; the replay guard because a stale step would reject codes from the new secret; the lock because an operator whose authenticator is gone has usually been failing into one; the sessions because a factor being reset may be a factor in the wrong hands.
- **No admin may approve a reset of their own factor.** `checkerId != makerId` governs who decides, not who is targeted, so this is a separate rule — checked under the row lock — without which a super-admin could clear their own second factor alone.
- **Decisions are type-scoped routes** (`/v1/admin/funding-requests/{id}/approve|reject`, `/v1/admin/totp-resets/{id}/approve|reject`), with `/v1/admin/pending-actions` remaining the unified read. The permission a decision needs depends on the row's type, and keeping it on the route keeps "who may approve what" visible in the diff that grants it; each decision re-asserts the type under the lock, which matters because the super-admin holds both checker halves.
- Residual, recorded rather than solved: a reset needs a **live checker**, so a deployment with one super-admin who loses their authenticator has no path — staffing at least two is an operational requirement. Two colluding operators (a compliance officer and a super-admin) can take over a third's account; four-eyes bounds unilateral action, not collusion, and what remains is a trail naming both.

- Not yet implemented: an **admin-initiated password reset** for a forgotten password — deferred deliberately, because Phase 1 has no out-of-band delivery channel for admins, so the checker would necessarily learn a working credential for another operator, which is the opposite of what four-eyes buys; the super-admin disable-and-recreate path remains for that case. Also not implemented: the "assist / view-as-customer" mode (deferred to Phase 2).
...
Implemented (Phase 1 remediation, Slice 10, ADR-0028): **field-level encryption exists, and admin TOTP secrets are its first consumer** — closing the gap ADR-0025 accepted. TOTP is the one secret that cannot be hashed, because RFC 6238 recomputes each code from the secret itself; it is therefore encrypted instead.

- **AES-256-GCM behind an `EncryptionPort`.** The local adapter holds data keys from configuration; a KMS-backed adapter implements the same two methods and changes no caller. That is ADR-0001's port-with-adapter rule applied to cryptography rather than to a third-party service.
- **Ciphertexts are self-describing** (`fenc$v1$keyId$iv$tag$ciphertext`), naming the key that sealed them — the same reasoning as the self-describing scrypt hash. Rotation is therefore configuration, not a migration: put the new key first, keep the old one, and new writes move while old rows still read.
- **The admin id is authenticated as additional data.** This defends something encryption alone does not: an attacker with database *write* access could otherwise copy a colleague's encrypted secret onto their own row and sign in with the colleague's authenticator, because the ciphertext is perfectly valid. Binding the row makes the tag check fail.
- **The keyring is required, with no default, and the API refuses to start without it.** A default would be a published key, and a plaintext fallback would let a deployment believe it had encryption while having none — the silent downgrade tenet 6 forbids. A wrong-sized key is rejected at boot rather than at first sign-in.
- **Reads still tolerate a pre-ADR-0028 plaintext secret**, re-sealing it on its next *successful* verification — the only path that has proven the stored value is the real secret. Writes never produce plaintext. The tolerance is removable once no such rows remain.

The honest limit: this defends a database read, which is the threat the gap actually named. An attacker holding both the process environment and the database holds the key and the ciphertext, and moving that boundary requires a KMS that never discloses key material — which is what the port is shaped for.
...
- Implemented (Phase 1 Slice 6, ADR-0024): a hash-chained `audit_log` records the sensitive actions now in place — P2P transfer, dev funding, SCA step-up, session revocation and refresh-reuse revocation, and account provisioning — each written **inside the audited action's own transaction**, so no action occurs without its audit. Records capture actor (customer or `system`), action, target, timestamp, and correlation id, with before/after only for mutations of mutable state (e.g. a session revocation) and **internal references only, never raw PII**, since the trail is un-erasable. Integrity is one global chain (`sha256(prev_hash + canonical(record))`, gap-free sequence) confirmed by `verifyAuditChain`; the database rejects UPDATE/DELETE via the same append-only triggers as the ledger, so any out-of-band edit or deletion of a past record breaks the chain. Admin read/verify surfaces over the trail arrive with admin RBAC (Slice 7).
- ~~Truncation of the most recent records is undetectable~~ — **closed 2026-08-07 (ADR-0031, Slice 11).** The chain proves that what it *contains* was not altered; it says nothing about what it no longer contains, so deleting the newest records left a shorter chain that verified perfectly. The head's `(seq, hash)` is now signed on an interval (default five minutes) and **published to the process log**, with a copy in an `audit_anchors` table.
  - **The published line is the control; the table is convenience.** Whoever can truncate the trail can delete the anchor rows in the same transaction. What survives is the copy that already left the host through log shipping, which cannot be unpublished. The table exists so that ordinary verification detects truncation automatically, with no operator action.
  - **Signing is Ed25519, not an HMAC** — anyone able to verify an HMAC can forge one, which would force every verifier inside the trust boundary of the signer. The point of an anchor is to be checkable by someone who has stopped trusting this system. The verification key is public, and is logged at startup so it can be pinned out of band. `SigningPort` is KMS-shaped like `EncryptionPort` (ADR-0001, ADR-0028), so an HSM adapter replaces it without touching a caller; every signature names its key, so rotation is configuration rather than a migration.
  - **`GET /v1/admin/audit/verify` now reports the chain and the tail separately**, and `ok` is their conjunction. `POST /v1/admin/audit/verify-anchor` checks the trail against an anchor the caller holds from the log archive — the path that still answers when the table was deleted alongside the records it attested to.
  - **`unanchored` is a distinct status and is not a failure.** A system that has not published yet and one whose anchors were all deleted are indistinguishable from inside the database, so the API reports "no claim checked" rather than implying either safety or alarm.
  - **Residual, recorded rather than solved:** this defends against database access, not against a host that also holds the signing key — the same boundary ADR-0028 draws for encryption, and it moves only when the key moves into a KMS or HSM. The truncation window is the publish interval: records appended since the last anchor remain deletable undetectably. And the guarantee now depends on log retention, which makes log retention a security control rather than an operational convenience.
- `AUDIT_ANCHOR_KEYS` is **required with no default**, like `ENCRYPTION_KEYS` and for a sharper version of the same reason: a default would be a published signing key, which lets anyone mint an anchor for a truncated chain, and making it optional would let a deployment appear to anchor while publishing nothing.
...
| Version | Date | Change |
|---|---|---|
| 0.10.0 | 2026-08-07 | Phase 1 remediation, Slice 11 — audit tail anchoring (§8, ADR-0031). Closes the deferral ADR-0024 recorded against itself, and the last correctness debt in the audit trail: the hash chain proves nothing it *contains* was altered, but deleting the newest records left a shorter chain that verified perfectly, so an insider could erase what they had just done. The head is now signed on an interval and **published to the process log**, with a copy in a table — and the distinction matters, because whoever can truncate the trail can delete the table rows in the same transaction. What survives is the copy that already left the host. Signing is Ed25519 rather than an HMAC, so an anchor is checkable by someone who has stopped trusting this system; the `SigningPort` is KMS-shaped, so an HSM adapter replaces it without touching a caller. Verification now reports chain and tail separately, `unanchored` is an explicit non-failure state, and `POST /v1/admin/audit/verify-anchor` verifies against an anchor held from a log archive. New setting `AUDIT_ANCHOR_KEYS` (required, no default) and `AUDIT_ANCHOR_INTERVAL_MS`. Migration `0013`. Residual: this defends the database, not a host holding the signing key, and the truncation window is the publish interval. |
| 0.9.0 | 2026-08-07 | Phase 1 remediation, Slice 10 Wave B — admin credential rotation and recovery (§3.2, ADR-0030). Closes the last ADR-0025 credential gap: an operator can now rotate their own password, re-proving both factors and cutting every other session they hold, and a lost second factor is recovered through a four-eyes reset rather than a direct database edit. The reset is the second registered `pending_admin_actions` type, which the generic table absorbed without a migration; its maker half is narrower than funding's, because funding credits a customer within a cap while a reset hands over a back-office identity. Two new invariants: the code spent rotating a password advances the same replay guard sign-in advances, so it cannot be spent twice; and no admin may approve a reset of their own factor, which `checkerId != makerId` does not cover because it governs who decides rather than who is targeted. Decisions moved to type-scoped routes so the permission each needs stays on the route rather than moving into a service. Deferred with a reason: an admin-initiated *password* reset, which without an out-of-band channel would hand the checker a working credential for another operator. |
| 0.8.0 | 2026-08-06 | Phase 1 remediation, Slice 10 Wave A — admin credential hardening. Closes the most serious open item in this document: **admin TOTP secrets are no longer stored unencrypted** (§6.2, ADR-0028). Field-level encryption now exists as an `EncryptionPort` with a local AES-256-GCM keyring adapter, shaped so a KMS adapter replaces it without touching a caller; ciphertexts are self-describing and name their key, so rotation is configuration rather than a migration; and the admin id is authenticated as additional data, which defeats grafting one operator's encrypted secret onto another's row — a privilege-escalation path encryption alone would have left open. The keyring is required with no default, because a default would be a published key and a plaintext fallback would let a deployment believe it had encryption while having none. Also closes two ADR-0025 gaps (§3.2, ADR-0029): **per-account lockout** counting both factors against one counter, which stops online guessing that the in-memory per-IP throttle only slowed; and **denied attempts are now audited**, written in their own transaction immediately after the failure — the documented exception to ADR-0024's atomicity rule, necessary because `verifyMfa` deliberately rolls its transaction back to protect a mistyped code, which is precisely why failed second-factor attempts previously went uncounted. Migration `0012`. |
